...
export default function IndexPage() {
  ...
  // Compartilhamento via WhatsApp a partir do simulador ativo
  const handleCompartilharWhatsAppAtual = () => {
    try {
      ...
      toast.success('Abrindo WhatsApp com o estudo atual...')
    } catch (err) {
      console.error(err)
      toast.error('Erro ao formatar mensagem para WhatsApp.')
    }
  }

  const handleCopiarResumoAtual = async () => {
    try {
      const mensagem = formatarMensagemWhatsApp({
        titulo: tituloSimulacao.trim() || nomeEmpreendimento,
        empreendimento: nomeEmpreendimento,
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#E3DFD6] pb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Botão Compartilhar WhatsApp do Estudo Atual */}
          <Button
            type="button"
            variant="outline"
            onClick={handleCompartilharWhatsAppAtual}
            className="rounded-xl border-[#25D366]/40 hover:border-[#25D366] bg-[#25D366]/5 hover:bg-[#25D366]/15 text-[#1b9a4a] text-xs font-semibold gap-1.5 transition-colors"
            title="Compartilhar simulação atual no WhatsApp"
          >
