...
import {
  ...
  extrairDadosPlanoDeSimulacaoRecord,
...
export default function SimulacoesPage() {
  ...
  const [copiadoPlanoId, setCopiadoPlanoId] = useState<string | null>(null)
  ...
  const handleCompartilharWhatsAppPlano = (sim: SimulacaoRecord) => {
    try {
      const dados = extrairDadosPlanoDeSimulacaoRecord(sim)
      const mensagem = formatarMensagemPlanoPagamento(dados)
      abrirWhatsAppComTexto(mensagem)
      toast.success('Abrindo WhatsApp com o plano de pagamento...')
    } catch (err) {
      console.error(err)
      toast.error('Não foi possível formatar o plano de pagamento para WhatsApp.')
    }
  }

  const handleCopiarPlano = async (sim: SimulacaoRecord) => {
    try {
      const dados = extrairDadosPlanoDeSimulacaoRecord(sim)
      const mensagem = formatarMensagemPlanoPagamento(dados)
      const ok = await copiarTextoParaClipboard(mensagem)
      if (ok) {
        setCopiadoPlanoId(sim.id)
        toast.success('Plano de pagamento copiado para a área de transferência!')
        setTimeout(() => setCopiadoPlanoId(null), 2500)
      } else {
              ...
              >
                ...
                <div className="p-4 pt-0 border-t border-[#E3DFD6]/60 flex items-center justify-between gap-2 mt-2">
                  <div className="flex items-center gap-1">
                    {/* Botão Compartilhar Resumo no WhatsApp */}
                    <Button
                      type="button"
                      variant="ghost"
                      size="icon"
                      ...
                      title="Compartilhar resumo de rentabilidade no WhatsApp"
                      aria-label="Compartilhar resumo de rentabilidade no WhatsApp"
                    >
                    ...
                    <Button
                      ...
                      title="Copiar resumo de rentabilidade em texto"
                    ...
                    {/* Botão Compartilhar Plano no WhatsApp */}
                    <Button
                      type="button"
                      variant="ghost"
                      size="icon"
                      onClick={() => handleCompartilharWhatsAppPlano(sim)}
                      className="h-8 w-8 text-[#0F6B4F] hover:text-[#0B5740] hover:bg-[#0F6B4F]/10 rounded-lg transition-colors"
                      title="Compartilhar plano de pagamento no WhatsApp"
                      aria-label="Compartilhar plano de pagamento no WhatsApp"
                    >
                    ...
                    <Button
                      ...
                      variant="ghost"
                      size="icon"
                      onClick={() => handleCopiarPlano(sim)}
                      className="h-8 w-8 text-[#5E6E64] hover:text-[#0F6B4F] hover:bg-[#0F6B4F]/10 rounded-lg transition-colors"
                      title="Copiar plano de pagamento em texto"
                      aria-label="Copiar plano de pagamento"
                    >
                      {copiadoPlanoId === sim.id ? (
                        <Check className="h-4 w-4 text-[#0F6B4F]" />
                      ) : (
